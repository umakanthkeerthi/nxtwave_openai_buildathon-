...
const Pharmacy = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            // Don't clear ÔÇö keep any local orders
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchInventory = async () => {
        ...
        try {
            ...
            const apiUrl = import.meta.env.VITE_API_URL;
            const res = await axios.get(`${apiUrl}/pharmacy/inventory?pharmacy_id=${selectedPharmacy.id}`);
    ...
    const confirmOrder = async () => {
        if (!deliveryAddress.trim()) {
            showToast("ÔÜá´©Å Please enter a valid delivery address.");
            return;
        }
        ...
        try {
            const apiUrl = import.meta.env.VITE_API_URL;
            const res = await axios.post(`${apiUrl}/pharmacy/orders`, orderPayload);
        ...
        setCart([]);
        setDeliveryAddress("");
        showToast("­ƒÄë Order placed successfully!");
        ...
        <div style={{
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    {orders.map(order => (
                                        <motion.div key={order.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                            ...
                                            <div style={{ fontWeight: '700', fontSize: '1.1rem', color: '#0f172a' }}>Ôé╣{order.total?.toFixed(2)}</div>
                                        </motion.div>
                                    ))}
                                </div>
                            ) : (
                                        ...
                                        <tbody>
                                            {orders.map(order => (
                                                <tr key={order.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                                    ...
                                                    <td style={{ padding: '12px', fontWeight: '600' }}>Ôé╣{order.total?.toFixed(2)}</td>
                                ...
                                {filteredProducts.map((product, index) => (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid #f1f5f9' }}>
                                            <span style={{ fontWeight: '700', color: '#0f172a', fontSize: '1.1rem' }}>Ôé╣{product.price.toFixed(2)}</span>
                                            <button
                                    ...
                                    <tbody>
                                        {filteredProducts.map((product, index) => (
                                            ...
                                            >
                                                ...
                                                <td style={{ padding: '16px', textAlign: 'right', fontWeight: '600', color: '#0f172a' }}>Ôé╣{product.price.toFixed(2)}</td>
                    ...
                    >
                                        ...
                                        <div key={item.id} style={{ display: 'flex', gap: '1rem', paddingBottom: '1rem', borderBottom: '1px solid #f1f5f9' }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    <span style={{ fontWeight: 'bold', color: 'var(--color-primary)' }}>Ôé╣{item.price.toFixed(2)}</span>
                        ...
                        <div style={{ marginTop: '2rem', borderTop: '2px solid #f1f5f9', paddingTop: '1rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', fontSize: '1.2rem', fontWeight: 'bold' }}>
                                <span>Total</span>
                                <span>Ôé╣{cartTotal.toFixed(2)}</span>
                            </div>
                            <button
            ...
            {toast && (
                <div style={{
                    ...
                    <span style={{ fontSize: '1.2rem' }}>Ô£à</span>
                    {toast}
                </div>
            )}
        </div>
