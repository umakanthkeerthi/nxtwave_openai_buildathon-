...
def step_1_create_summary():
    ...
    try:
        resp = requests.post(f"{BASE_URL}/save_summary", json=payload, timeout=30)
...
def step_2_book_appointment():
    ...
    try:
        resp = requests.post(f"{BASE_URL}/book_appointment", json=payload, timeout=10)
        print(f"Status: {resp.status_code}")
        print(f"Response: {resp.json()}")
...
def step_3_doctor_submit_prescription():
    ...
    try:
        resp = requests.post(f"{BASE_URL}/upload_record", json=payload, timeout=10)
        print(f"Status: {resp.status_code}")
        print(f"Response: {resp.json()}")
...
def step_4_verify_records():
    ...
    try:
        resp = requests.get(f"{BASE_URL}/get_records?patient_id={PATIENT_ID}", timeout=10)
        ...
        if "PRESCRIPTION" in types_found and "AI_SUMMARY_DOCTOR" in types_found:
             print("\n✅ SUCCESS: Summary (AI_SUMMARY_DOCTOR) and Prescription both found!")
        elif "PRESCRIPTION" in types_found and "summary" in types_found:
...
if __name__ == "__main__":
    print("=== STARTING END-TO-END VERIFICATION (PORT 8005) ===")
